...
const addGrade = (students) => {
  const modifiedStudents = students.map((student) => {
    ...
    const newStudentWithGrades = { name: name, marks, grade: getGrade(marks) };
    return newStudentWithGrades;
  });
...
const cartCalculator = (products) => {
  let totalitems = 0;
  let totalcost = 0;
  //destucturing and using for...of loop
  for (let product of products) {
    const { price, qty } = product;
    totalitems += qty;
    totalcost += price * qty;
  }
  return `Total: ${totalitems || 0} items, ${totalcost || 0} Taka`;
};
const product = [
  { name: "Pen", price: 100, qty: 1 },
  { name: "Notebook", price: 100, qty: 2 },
  { name: null, price: null, qty: null },
];
console.log(cartCalculator(product));
